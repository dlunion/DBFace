...
def _interpolate(name, dim, interpolate_mode):
    def symbolic_fn(g, input, output_size, align_corners=None):
        ...
        if interpolate_mode == "bilinear":
            ...
            return g.op("Plugin", input, name_s="Bilinear", info_s=json.dumps({
                "scale_factor": [2, 2]
            }))
        else:
            return g.op("Upsample", input, scales_f=[1, 2, 2], mode_s=interpolate_mode)

        #return g.op("Upsample", input, scales_f=[1, 2, 2], mode_s="nearest")
        # return g.op("Resize",
        #             input,
